...
void setup(){
 ...
 Wire.begin();
 while(!DSERIAL);
 //DSERIAL.print(ams5600.getBurnCount());
 printMenu();
}
void printMenu()
{
  for(int i =0; i<20;i++)
  DSERIAL.println();
  DSERIAL.println("AS5600 DSERIAL Interface Program");
  DSERIAL.println("");
  if(lastResponse.length()>0)
  {
    DSERIAL.println(lastResponse);
    DSERIAL.println("");
  }
  DSERIAL.print("1 - Set start position\t|  "); DSERIAL.println(" 6 - get end position");
  DSERIAL.print("2 - Set end position\t|  ");   DSERIAL.println(" 7 - get raw angle");
  DSERIAL.print("3 - Set max angle range\t|  ");  DSERIAL.println(" 8 - get scaled angle");
  DSERIAL.print("4 - Get max angle range\t|  ");  DSERIAL.println(" 9 - detect magnet");
  DSERIAL.print("5 - Get start position \t\t|  ");     DSERIAL.println("10 - get magnet strength");
  DSERIAL.println();
  DSERIAL.println("Number of burns remaining: "); DSERIAL.println(String(3 - ams5600.getBurnCount()));
  DSERIAL.println("96 - Burn Angle");
  DSERIAL.println("98 - Burn Settings (one time)");
}
...
void loop()
{
  ...
  if (DSERIAL.available() > 0)
  {
    ...
    DSERIAL.readBytes(incomingByteBuffer,2);
      ...
      case 10:
      {
        ...
        if(ams5600.detectMagnet()==1)
        {
          ...
          if(magStrength == 1)
            ...
            DSERIAL.print("Current Magnitude: ");
            DSERIAL.println(ams5600.getMagnitude());
          }
